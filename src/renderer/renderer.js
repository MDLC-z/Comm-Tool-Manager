...
async function showView(viewName) {
    ...
    if (currentView && currentView.id === 'view-config' && viewName !== 'config') {
    ...
    if (appState.backgroundAllPages && appState.backgroundImage && imageCache.has(appState.backgroundImage)) {
...
function renderCommsGrid() {
    ...
    if (sortedComms.length === 0) {
        ...
        const noCommsDesc = window.t ? window.t('comms.no_comms_desc') : 'Create your first comm by clicking the "+" button';
    ...
    commsGrid.innerHTML = sortedComms.map(comm => {
        ...
        return `
            <div class="comm-card ${pinnedClass}" onclick="openCommEditor('${comm.id}')">
                <div class="comm-header">
                    <div class="comm-title">
                        ${comm.pinned ? '📌 ' : ''}"${comm.title}"
                    </div>
                    <div class="comm-actions">
                        <button class="pin-btn" onclick="togglePinComm('${comm.id}', event)">
                            ${comm.pinned ? '📌' : '📍'}
                        </button>
                        <div class="comm-price ${priceClass}">${formattedPrice}</div>
                    </div>
                </div>
                <div class="comm-commissioner">${forText}: "${comm.commissioner}"</div>
                <div class="comm-meta">
                    <span class="status-badge ${comm.status}">${getStatusText(comm.status)}</span>
                    <span class="priority-badge ${comm.priority}">${getPriorityText(comm.priority)}</span>
...
function renderFilteredComms(comms) {
    ...
    if (sortedComms.length === 0) {
        const noResultsText = window.t ? window.t('comms.no_results') : 'No comms found';
        const noResultsDesc = window.t ? window.t('comms.no_results_desc') : 'Try different search filters';
...
async function togglePinComm(commId, event) {
    ...
    try {
        ...
        alert('Error updating comm: ' + error.message);
    }
}
...
async function autoSaveConfig() {
    ...
    try {
        ...
        console.log('Configuration auto-saved successfully. Language:', currentLanguage);
...
async function saveConfig() {
    ...
    try {
        ...
        if (saveStatus) {
            saveStatus.textContent = window.t ? window.t('alerts.saving') : 'Saving changes...';
        ...
        if (saveStatus) {
            saveStatus.textContent = window.t ? window.t('alerts.saved') : 'Settings saved';
        ...
        setTimeout(() => {
            if (saveBtn) {
                ...
                saveBtn.innerHTML = window.t ? window.t('settings.save_changes') : 'Save Changes';
            }
            if (saveStatus) {
        ...
        if (saveStatus) {
            saveStatus.textContent = (window.t ? window.t('alerts.error') : 'Error') + ': ' + error.message;
    ...
    try {
        if (appState.backgroundImage) {
            ...
            if (imageCache.has(appState.backgroundImage)) {
                ...
                applyBackgroundImage(imageCache.get(appState.backgroundImage), appState.backgroundAllPages);
            } else {
                console.log('Loading background image from file system');
                const imageData = await window.electronAPI.loadBackgroundImage(appState.backgroundImage);
...
function applyBackgroundImage(imageData, allPages = false) {
    ...
    document.body.classList.remove('background-all-pages', 'background-welcome-only', 'has-background-image');
    ...
    if (imageData) {
        ...
        appBackground.style.background = 'linear-gradient(135deg, var(--primary-color) -500%, var(--bg-color) 100%)';
    }
...
async function changeBackgroundImage() {
    ...
    input.onchange = async (e) => {
        ...
        if (file.size > 10 * 1024 * 1024) {
            alert(window.t ? window.t('alerts.image_too_large', { name: file.name }) : `The image ${file.name} is too large (max 10MB)`);
            return;
        }
        ...
        try {
            const saveStatus = document.getElementById('save-status');
            if (saveStatus) {
                saveStatus.textContent = window.t ? window.t('alerts.saving') : 'Processing image...';
                saveStatus.className = 'save-status saving show';
            }
            ...
            if (saveStatus) {
                saveStatus.textContent = window.t ? window.t('alerts.background_changed') : 'Background changed successfully';
                saveStatus.className = 'save-status show';
                setTimeout(() => { 
                    saveStatus.className = 'save-status'; 
                }, 3000);
            } else {
                alert(window.t ? window.t('alerts.background_changed') : 'Background changed successfully');
            }
            ...
            const saveStatus = document.getElementById('save-status');
            if (saveStatus) {
                saveStatus.textContent = window.t ? window.t('alerts.background_error') : 'Error changing background';
                saveStatus.className = 'save-status error show';
                setTimeout(() => { 
                    saveStatus.className = 'save-status'; 
                }, 3000);
            } else {
                alert((window.t ? window.t('alerts.background_error') : 'Error changing background') + ': ' + error.message);
            }
        }
    };
...
async function removeBackgroundImage() {
    ...
    if (confirm(window.t ? window.t('alerts.confirm_delete_image') : 'Are you sure you want to delete this image?')) {
        try {
            ...
            alert(window.t ? window.t('alerts.background_removed') : 'Custom background removed');
        } catch (error) {
    ...
    .then(() => {
        if (appState.backgroundImage) {
            ...
            if (imageCache.has(appState.backgroundImage)) {
                ...
                window.electronAPI.loadBackgroundImage(appState.backgroundImage)
                    .then(imageData => {
                        imageCache.set(appState.backgroundImage, imageData);
                        applyBackgroundImage(imageData, appState.backgroundAllPages);
                    })
...
function sortComms(comms, sortBy) {
    ...
    switch(sortBy) {
        case 'newest':
            sortedUnpinned.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
            break;
        case 'oldest':
            sortedUnpinned.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
            break;
        ...
        case 'priority': // Por prioridad (High > Medium > Low)
            ...
            sortedUnpinned.sort((a, b) => priorityOrder[b.priority] - priorityOrder[a.priority]);
            break;
        default:
            sortedUnpinned.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    }
...
function updateSortButton() {
    ...
    const sortLabels = {
        ...
        'deadline_asc': window.t ? window.t('sort.deadline_closest') : 'Deadline (Closest)',
        'deadline_desc': window.t ? window.t('sort.deadline_farthest') : 'Deadline (Farthest)',
        'price_asc': window.t ? window.t('sort.price_low_high') : 'Price (Low to High)',
        'price_desc': window.t ? window.t('sort.price_high_low') : 'Price (High to Low)',
...
function getCommEditorHTML() {
    ...
    const titlePlaceholder = window.t ? window.t('editor.title_placeholder') : 'Comm title';
    const commissionerText = window.t ? window.t('editor.commissioner') : 'Commissioner';
    const commissionerPlaceholder = window.t ? window.t('editor.commissioner_placeholder') : 'Commissioner name';
    const priceText = window.t ? window.t('editor.price') : 'Price (USD)';
    const pricePlaceholder = window.t ? window.t('editor.price_placeholder') : '0 for free request';
    ...
    const descriptionText = window.t ? window.t('editor.description') : 'Description';
    const descriptionPlaceholder = window.t ? window.t('editor.description_placeholder') : 'Detailed work description...';
    const referencesText = window.t ? window.t('editor.references') : 'References';
    const addImagesText = window.t ? window.t('editor.add_images') : '+ Add Images';
    const openFolderText = window.t ? window.t('editor.open_folder_btn') : 'Open Folder';
    const noImagesText = window.t ? window.t('editor.no_images') : 'No images yet. Add some references.';
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="comm-status">
                                ...
                                <option value="progress">${getStatusText('progress')}</option>
                        ...
                        <div class="form-group">
                            ...
                            <select id="comm-type">
                                ...
                                <option value="translation">${getTranslationText('translation')}</option>
                                <option value="web">${getTranslationText('web_design')}</option>
                            </select>
                        </div>
                    </div>
...
function loadCommData(commId) {
    ...
    const editText = window.t ? window.t('editor.edit_comm', { title: comm.title }) : `Editing: ${comm.title}`;
...
async function saveComm() {
    ...
    const commissioner = document.getElementById('comm-commissioner').value.trim();
    ...
    const validationTitle = window.t ? window.t('alerts.validation_title') : 'Title is required';
    const validationCommissioner = window.t ? window.t('alerts.validation_commissioner') : 'Commissioner name is required';
    //const validationPrice = window.t ? window.t('alerts.validation_price') : 'Price must be greater than 0';
    const validationDeadline = window.t ? window.t('alerts.validation_deadline') : 'Deadline is required';
    ...
    */
    ...
    const commData = {
        ...
        pinned: appState.currentEditingComm ? 
            appState.comms.find(c => c.id === appState.currentEditingComm)?.pinned || false : false,
        createdAt: appState.currentEditingComm ? 
            appState.comms.find(c => c.id === appState.currentEditingComm)?.createdAt || new Date().toISOString() : 
    ...
    try {
        if (!appState.currentEditingComm) {
            ...
            const index = appState.comms.findIndex(c => c.id === appState.currentEditingComm);
            if (index !== -1) {
        ...
        const successMessage = window.t ? window.t('alerts.comm_saved') : 'Comm saved successfully';
        ...
        alert('Error saving comm: ' + error.message);
    }
}
...
async function deleteComm() {
    ...
    const confirmMessage = window.t ? window.t('alerts.confirm_delete_comm') : 'Are you sure you want to delete this comm? This action cannot be undone.';
    ...
    if (confirm(confirmMessage)) {
        ...
        if (index !== -1) {
            try {
                ...
                const successMessage = window.t ? window.t('alerts.comm_deleted') : 'Comm deleted successfully';
                alert(successMessage);
                closeEditor();
                renderCommsGrid();
                ...
                alert('Error deleting comm: ' + error.message);
            }
        }
    }
}
...
async function cleanupOrphanedComms() {
    if (confirm('This will delete all comm folders that are not in the JSON file. Continue?')) {
        try {
            const deletedCount = await window.electronAPI.cleanupOrphanedComms();
            alert(`Cleanup completed: ${deletedCount} orphaned folders deleted`);
        } catch (error) {
...
async function addReferenceImage() {
    ...
    input.onchange = async (e) => {
        ...
        if (!currentCommId) {
            const alertText = window.t ? window.t('alerts.save_comm_first') : 'First you need to save the comm before adding references';
            alert(alertText);
            return;
        }
        ...
        for (const file of files) {
            try {
                ...
                */
                ...
                const fileExtension = file.name.split('.').pop();
                const filename = `ref_${Date.now()}_${Math.random().toString(36).substr(2, 9)}.${fileExtension}`;
                ...
                // Guardar referencia
                await window.electronAPI.saveCommReference(currentCommId, fileData, filename, fileType);
                ...
                const alertText = window.t ? window.t('alerts.error_adding_file', { name: file.name }) : `Error adding ${file.name}`;
...
function updateReferenceGallery() {
    ...
    if (!appState.currentReferences || appState.currentReferences.length === 0) {
        const noRefsText = window.t ? window.t('editor.no_images') : 'No references yet. Add images, videos or files.';
        gallery.innerHTML = `
            <div class="empty-gallery">
                <p>${noRefsText}</p>
            </div>
    ...
    gallery.innerHTML = appState.currentReferences.map((ref, index) => {
        if (ref.type === 'image' && ref.data) {
            ...
            return `
                <div class="gallery-item" data-type="file">
                    <div class="file-icon" onclick="openReferenceFile('${ref.name}')">
...
function getFileType(filename) {
    ...
    const typeMap = {
        // Videos
        'mp4': 'MP4', 'avi': 'AVI', 'mov': 'MOV', 'wmv': 'WMV', 'webm': 'WEBM', 'mkv': 'MKV',
        // Documentos
        'pdf': 'PDF', 'doc': 'DOC', 'docx': 'DOCX', 'txt': 'TXT',
        // Diseño
        'psd': 'PSD', 'ai': 'AI', 'afdesign': 'Affinity',
...
async function removeReference(filename) {
    if (!confirm(window.t ? window.t('alerts.confirm_delete_image') : 'Are you sure you want to delete this reference?')) return;
    ...
    try {
        ...
        alert('Error removing reference: ' + error.message);
    }
}
...
async function openReferenceFile(filename) {
    try {
        const currentCommId = appState.currentEditingComm;
        if (!currentCommId) {
            alert('No comm selected');
            return;
        }
        
        const filePath = await window.electronAPI.getReferenceFilePath(currentCommId, filename);
        await window.electronAPI.openFile(filePath);
    } catch (error) {
        console.error('Error opening file:', error);
        const errorText = window.t ? window.t('alerts.error_opening_file') : 'Error opening file';
...
function openReferenceViewer(index) {
    ...
    if (ref.type === 'image' && ref.data) {
        ...
        const imageRefs = appState.currentReferences.filter(r => r.type === 'image' && r.data);
...
function updateImageGallery() {
    ...
    gallery.innerHTML = appState.currentImages.map((img, index) => `
...
function nextImage() {
    ...
    appState.currentImageIndex = (appState.currentImageIndex + 1) % appState.currentImages.length;
    openImageViewer(appState.currentImageIndex);
}
...
function previousImage() {
    ...
    appState.currentImageIndex = appState.currentImageIndex === 0 ? 
        appState.currentImages.length - 1 : appState.currentImageIndex - 1;
    openImageViewer(appState.currentImageIndex);
}
...
function deleteCurrentImage() {
    ...
    const confirmMessage = window.t ? window.t('alerts.confirm_delete_image') : 'Are you sure you want to delete this image?';
    ...
    if (confirm(confirmMessage)) {
        ...
        if (appState.currentImages.length === 0) {
            ...
            appState.currentImageIndex = Math.min(appState.currentImageIndex, appState.currentImages.length - 1);
            openImageViewer(appState.currentImageIndex);
        }
    }
}
...
function toggleZoom() {
    const viewerImage = document.getElementById('viewer-image');
    if (viewerImage) {
        const currentScale = viewerImage.style.transform.includes('scale(2)') ? 1 : 2;
...
function confirmDeleteAll() {
    const confirmMessage = window.t ? window.t('alerts.confirm_delete_all') : 'Are you sure you want to delete ALL data? This action cannot be undone.';
    
    if (confirm(confirmMessage)) {
        window.electronAPI.deleteAllData()
            .then(() => {
                const successMessage = window.t ? window.t('alerts.all_data_deleted') : 'All data has been deleted';
                alert(successMessage);
                location.reload();
            })
            .catch(error => {
...
async function openCommFolder() {
    ...
    if (!currentCommId) {
        alert('You need to save the comm first');
        return;
    }
    
    try {
        // Obtener la ruta de la carpeta de referencias
        const referencesFolder = await window.electronAPI.getReferenceFolderPath(currentCommId);
...
async function changeBackgroundVideo() {
    ...
    input.onchange = async (e) => {
        ...
        if (file.size > 500 * 1024 * 1024) {
            alert(window.t ? window.t('alerts.video_too_large') : 'The selected video is too large. Maximum size is 500MB.');
            return;
        }
        ...
        try {
            const saveStatus = document.getElementById('save-status');
            if (saveStatus) {
                saveStatus.textContent = window.t ? window.t('alerts.saving_video') : 'Saving video...';
                saveStatus.className = 'save-status show';
            }
...
async function applyBackgroundVideo(filename, allPages = false) {
    ...
    try {
        ...
        video.onloadeddata = () => {
            ...
            document.body.classList.remove('background-all-pages', 'background-welcome-only');
            if (allPages) {
                document.body.classList.add('background-all-pages');
            } else {
                document.body.classList.add('background-welcome-only');
